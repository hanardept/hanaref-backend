...
module.exports = {
    async getItems(req, res) {
        ...
        try {
            const items = await Item.aggregate([
                ...
                {
                    $match: search ? { $or: [ { name: { $regex: search, $options: 'i' } }, { cat: { $regex: search } }, { "models.name": { $regex: search, $options: 'i' } }, { "models.cat": { $regex: search, $options: 'i' } } ] } : {}
                },
                {
            ...
            ]).sort('name').skip(page*20).limit(20);
    ...
    async editItem(req, res) {
        ...
        try {
            await Item.findOneAndUpdate({ cat: req.params.cat}, {
                name: name,
                cat: cat,
                sector: sector,
                department: department,
